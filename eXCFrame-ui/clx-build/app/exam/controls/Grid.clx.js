				...
				(function(container){
					...
					(function(container){
						...
						(function(container){
							...
							output_7.value = "1) selectionUnit : 그리드를 선택할 수 있는 단위를 설정하는 속성입니다. 기본값은 row이고 cell,column 단위로 변경이 가능합니다.\r\n\r\n2) selectionMulti : 그리드의 다중 선택 여부를 설정하는 속성입니다. 기본값은 sing이고 none,multi로 변경이 가능합니다.";
						...
						(function(container){
							...
							output_9.value = "1) resizableColumn : 그리드 헤더의 사이즈 조절 가능 여부를 설정하는 속성입니다. 기본값은 none 이고 all,columnIndex로 변경이 가능합니다.\r\n\r\n[none] : 모든 컬럼이 크기 변경이 되지 않습니다.\r\n[all] : 모든 컬럼이 클기 변경이 가능합니다.\r\n[columnindex] : 특정 컬럼 인덱스를 지정시, 해당 컬럼만 크기 조절이 가능합니다.\r\n\r\n2) columnMovable : 그리드 헤더의 이동 가능 여부를 설정하는 속성입니다. 기본값은 false이며 true로 변경시 그리드 헤더의 위치 변경이 가능합니다.";
						...
						(function(container){
							...
							output_11.value = "1) leftSplit/rightSplit : 그리드의 왼쪽(오른쪽)으로 셀을 고정 할 수 있게 설정하는 속성입니다. 그리드의 모든 셀을 leftSplit 또는 rightSplit으로 설정하는 경우 세로 스크롤이 보이지 않고, 그리드 내에 방향키를 통한 이동 및 스크롤이 동작하지 않습니다.\r\n\r\n2) leftSplitWidth/rightSplitWidth : 그리드의 leftSplit또는 rightSplit 으로 고정된 영역의 너비를 지정하는 속성입니다. ";
						...
						(function(container){
							...
							output_17.value = "1) 하단의 그리드 헤더 컬럼 사이를 드래그 합니다.\r\n2) 컬럼의 크기가 변경됩니다.\r\n3) 그리드 헤더를 옆 헤더로 드래그 앤 드롭합니다.\r\n4) 컬럼의 위치가 변경됩니다.";
							...
							output_19.value = "resizableColumn = all\r\ncolumnMovable = true";
							output_19.style.css({
								"background-color" : "transparent",
								"border-right-style" : "none",
								"border-left-style" : "none",
					...
					(function(container){
						...
						(function(container){
							...
							output_23.value = "그리드의 푸터에서 총합을 계산하는 함수입니다. getSum(\"컬럼명\") 으로 사용이 가능합니다.";
						...
						(function(container){
							...
							output_25.value = "그리드에서 rowgroup사용시 해당 그룹을 접을 수 있도록 설정할 수 있는 속성입니다. 기본값은 false 이고, true로 변경시 그룹을 접을 수 있게 됩니다.\r\n\r\n2) startCollapsible ";
						...
						(function(container){
							...
							output_35.value = "1) 하단의 그리드 푸터의 좌측 숫자는 가격의 총합 입니다.\r\n- getSum(\"price\")\r\n2) 하단의 그리드 푸터의 우측 숫자는 할인된 가격의 총합입니다.\r\n- getSum(\"ratedPrice\")";
					...
					(function(container){
						...
						(function(container){
							...
							output_46.value = "그리드를 클릭 했을 때 진입하는 모드를 설정할 수 있는 속성입니다. 기본값은 select 이고 edit으로 변경 시 클릭과 동시에 그리드 편집모드로 진입하게 됩니다.";
						...
						(function(container){
							...
							output_48.value = "1) filterble : 그리드의 필터링 여부를 설정하는 속성입니다. 그리드 헤더셀에서 설정 할 수 있습니다. 기본값은 false이며 true로 변경시 그리드 헤더에 필터아이콘을 클릭하여 필터링을 할 수 있습니다.\r\n\r\n2) sortable : 그리드의 정렬 가능 여부를 설정하는 속성입니다. 그리드 헤더셀에서 설정 할 수 있습니다. 기본값은 false이며 true로 변경시 그리드 헤더에 정렬아이콘을 클릭하여 정렬을 할 수 있습니다.";
						...
						(function(container){
							...
							output_56.value = "1) 그리드 헤더 중 숫자 컬럼에 필터 아이콘이 보입니다.\r\n2) 필터 아이콘을 클릭하면 필터창 우측의 버튼의 순서가 취소,확인 으로 되어 있습니다.\r\n3) 그리드의 헤더를 클릭합니다.\r\n4) 행들이 정렬되는 것을 확인 할 수 있습니다.";
					...
					(function(container){
						...
						(function(container){
							...
							output_60.value = "그리드에서 Surpress된 디테일 셀을 표현하는 방법을 설정하는 속성입니다. 기본값은 split(분리형) 이며 merge(병합형) 으로 변경이 가능합니다.\r\n\r\n[split] : 같은 값이 반복될 때 처음 한번만 값을 출력하고 나머지 값은 생략합니다.(default) \r\n[merged] : 같은 값이 반복될 때 반복되는 값의 셀들을 병합하는 효과를 제공합니다.";
						...
						(function(container){
							...
							output_62.value = "그리드의 디테일 셀에서 설정이 가능한 속성이고 바로 직전의 로우의 값과 일치하는 텍스트 숨김여부를 설정하는 속성입니다. 기본값은 fasle이며 true로 변경시 셀 병합효과를 제공 받을 수 있습니다. 디자인 시점의 detail 행이 2줄 이상인 경우 , detail 전체 행의 수만큼 rowSpan을 가지고 있다면 suppress가 지원되고, 개별 행으로 되어 있다면 suppress를 지원하지 않습니다.";
						...
						(function(container){
							...
							output_64.value = "그리드에서 병합할 셀의 cellIndex를 리턴하는 표현식입니다. 표현식 실행결과 반환되는 cellIndex의 셀은 현재 셀보다 우측에 위치해야 합니다.(현재 셀의 collIndex보다 크거나 같은 collIndex를 갖는 셀이어야 합니다.)";
						...
						(function(container){
							...
							output_68.value = "1) 하단의 그리드에서 1,5번째 컬럼이 가격부터 할인된 가격까지 병합되어 있는 것을 확인할 수 있습니다.\r\n2) 하단의 그리드에서 상품그룹에 생활용품과 가전제품이 4개행씩 병합되어 있는 것을 확인할 수 있습니다.";
