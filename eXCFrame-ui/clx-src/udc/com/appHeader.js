...
function onBtnLayoutMouseenter(e) {
	var btnLayout = e.control;
	var vsAppFullId = btnLayout.getAppInstance().getRootAppInstance().id;
	var aryAppId = vsAppFullId.split('/');
	var appId = aryAppId[aryAppId.length - 1].split('$')[0];
	var buttonRect = btnLayout.getActualRect();
		...
		dialogManager.openDialog("app/exam/layout/imgViewer",
			...
			function(dialog) {
				dialog.ready(function(dialogApp) {
					dialogApp.setAppProperty('src', 'app/exam/layout/img/' + window.location.pathname.split('/')[6] + '.png');
